
# This is a basic python process for smartcare

print("Welcome to SmartCare: Community Clinic Appointment Booking System!")

# First Appointment
patient1_name = 'Alice Smith'
practitioner1_name = 'Dr. John Doe'
appointment1_time = '2024-07-20 10:00 AM'

print(f"Patient: {patient1_name} | Practitioner: {practitioner1_name} | Time: {appointment1_time}")

# Second Appointment
patient2_name = 'Bob Johnson'
practitioner2_name = 'Dr. Jane Roe'
appointment2_time = '2024-07-20 11:30 AM'
print(f"Patient: {patient2_name} | Practitioner: {practitioner2_name} | Time: {appointment2_time}")


appointments = []

def book_appointments(patient_name, practitioner_name, appointment_time):
    if not patient_name:
        raise ValueError("Patient name cannot be empty")
    appointment = {
            "patient" :patient_name,
            "practitioner" :practitioner_name,
            "time" :appointment_time
        }
    appointments.append(appointment)

def display_appointments():
    if not appointments:
        print("No appointments available")
        return
    for appointment in appointments:
        print(f"Patient: {appointment['patient']} | Practitioner: {appointment['practitioner']} | Appointment: {appointment['time']}")

print("Welcome to the SmartCare Appointment Booking System!")
book_appointments('Alice Smith', 'Dr. John Doe', '2024-07-20 10:00AM')
book_appointments('Bob Johnson', 'Dr. Jane Ron', '2024-07-20 11:30AM')
display_appointments()

# AI verion of simple appointment storage using lists and dictionaries

appointments = []

def add_appointment(patient, practitioner, time):
    """Store a basic appointment record."""
    record = {
        "patient": patient,
        "practitioner": practitioner,
        "time": time
    }
    appointments.append(record)

def show_all():
    """Display all stored appointments."""
    for item in appointments:
        print(f"{item['patient']} with {item['practitioner']} at {item['time']}")

# Example usage
add_appointment("Alice Smith", "Dr. John Doe", "10:00 AM")
add_appointment("Bob Johnson", "Dr. Jane Roe", "11:30 AM")
show_all()